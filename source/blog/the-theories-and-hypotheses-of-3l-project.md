...
假设通常无法得到证明。理论已经被证明，但是解决方案可能会改变。这些清单没有特别的顺序。
...
- 系统应该将实现细节与所有级别的应用程序开发完全分开，包括核心“OS”的特性开发。见“描述符...”一节；
...
**假设**：核心系统，即必须在硬件中被编译或实现的部分，受益于体积**小**。
- 使系统更具扩展性；
- 更容易进行实验；
- 更容易做到可靠；
- 更少的缺陷；
...
只要在最低层的设计上稍加注意，所有语言都可以被编译成相同的基础语言，从而实现无缝集成。
由于**小核心**，这大大简化了操作。
...
- 人们不会自然地从 API 的角度思考问题，但是语言确实是自然产生的；
- 库的各个层很难保持一致；
- DSL 会产生更一致的结果；

> 译者思考：库和框架使用越多，就越容易产生各种bug，调用的层次也越来越沉，系统也会越来越复杂，
...
- 直接植根于[数学](https://en.wikipedia.org/wiki/Lambda_calculus)（译注：这里指 Lambda 表达式）：
...
- 更好的硬件已经被开发出来；
- （假设）通过使用针对汇编语言和 C 语言以外的语言而设计的更好的架构，可以在不花很多钱的情况下击败英特尔和它的盟友；
